...
class BaseRepl(Repl):
    ...
    def __init__(
        ...
        config is a vypl config.Struct with config attributes
    ...
    def _schedule_refresh(self, when: float):
        """Arrange for the vypl display to be refreshed soon.
    ...
    def request_refresh(self):
        """Request that the vypl display to be refreshed soon."""
    ...
    def process_control_event(self, e: events.Event) -> bool | None:
        if isinstance(e, vyplevents.ScheduledRefreshRequestEvent):
            pass

        elif isinstance(e, vyplevents.RefreshRequestEvent):
        ...
        elif isinstance(e, vyplevents.RunStartupFileEvent):
            try:
                self.startup()
        ...
        elif isinstance(e, vyplevents.UndoEvent):
            self.undo(n=e.n)
    ...
    def process_simple_keypress(self, e: str):
        if e in ("<Ctrl-j>", "<Ctrl-m>", "<PADENTER>", "\n", "\r"):
            ...
            while self.fake_refresh_requested:
                self.fake_refresh_requested = False
                self.process_event(vyplevents.RefreshRequestEvent())
    ...
    def send_session_to_external_editor(self, filename=None):
        """
        Sends entire vypl session to external editor to be edited. Usually bound to F7.
        """
    ...
    def unhighlight_paren(self):
        ...
        if self.highlighted_paren is not None and self.config.syntax:
            ...
            new = vyplparse(pygformat(saved_tokens, self.formatter))
...
>"""
    ...
    def reprint_line(self, lineno, tokens):
        ...
        if self.config.syntax:
            self.display_buffer[lineno] = vyplparse(
    ...
    def reevaluate(self, new_code=False):
        ...
        self.process_event(vyplevents.RunStartupFileEvent())
    ...
    def initialize_interp(self) -> None:
        ...
        self.coderunner.interp.runsource(
            "from vypl.curtsiesfrontend._internal import _Helper\n"
        )
    ...
    def getstdout(self) -> str:
        """
        Returns a string of the current vypl session, wrapped, WITH prompts.
        """
...
vypl -i your_script.py runs a file in interactive mode
...
"""
        ...
        return f"vypl-curtsies version {__version__} using curtsies version {curtsies_version}\n{help_message}"
