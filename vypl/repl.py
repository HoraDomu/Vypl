...
class Interpreter(code.InteractiveInterpreter):
    """Source code interpreter for use in vypl."""

    vypl_input_re = LazyReCompile(r"<vypl-input-\d+>")
    ...
    def __init__(
        ...
        on a caught syntax error. The purpose for this in vypl is so that
    ...
    def showsyntaxerror(self, filename: str | None = None, **kwargs) -> None:
        ...
        if filename and exc_type is SyntaxError and value is not None:
            ...
            if self.vypl_input_re.match(filename):
                args[0] = "<input>"
...
class Repl(metaclass=abc.ABCMeta):
    ...
    def __init__(self, interp: Interpreter, config: Config) -> None:
        ...
        config is a populated vypl.config.Struct.
        """
        self.config = config
        self.cut_buffer = ""
    ...
    def edit_config(self):
        ...
        try:
            if self.open_in_external_editor(self.config.config_path):
                self.interact.notify(
                    _(
                        "vypl config file edited. Restart vypl for changes to take effect."
                    )
                )
        except OSError as e:
